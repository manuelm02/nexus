...
|MinIO      |Mac Studio 本地 Docker（或 NAS）|Tailscale 内网 / 域名反代|
...
# MinIO（Mac 本地 / NAS，Tailscale 访问）
nexus:
  minio:
    endpoint: ${MINIO_ENDPOINT:http://100.x.x.x:9000}
...
MinIO         → 文件对象存储（Mac 本地 / NAS，Tailscale 访问）
...
|文件存储             |MinIO       |S3 / 本地             |改 `file_storage` 字段值 + MinioClient|
...
1. 完整 `package.json`
1. 项目初始化命令序列
1. Flyway V1-V4 完整 SQL 文件
1. 所有 Controller 的 Request/Response DTO 定义
1. 环境变量清单（必填 / 可选 / 默认值 / 说明）
