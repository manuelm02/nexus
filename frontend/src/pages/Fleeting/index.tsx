...
import { fleetingApi } from '../../api/fleeting.api'
...
export default function FleetingPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['fleeting'],
    queryFn: () => fleetingApi.list(),
  })

  const createMutation = useMutation({
    mutationFn: () => fleetingApi.create({ title: title || undefined, content }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['fleeting'] })
      setContent('')
      setTitle('')
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => fleetingApi.delete(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['fleeting'] }),
  })

  const items: Fleeting[] = data?.data?.data ?? []
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-xl font-semibold">Fleeting</h1>
