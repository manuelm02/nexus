...
import { todayString } from '../todo.shared'
...
// TodoDatePicker 统一 ToDo 内所有日期入口，避免原生 date input 破坏 Nexus 的视觉一致性。
export function TodoDatePicker({ value, onChange, allowClear = false, showQuickChips = false, compact = false, invalid = false, placeholder = '年 / 月 / 日' }: {
  value?: string
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    if (nextOpen) setVisibleMonth(selectedDate ?? parseDateKey(today) ?? new Date())
    setOpen(nextOpen)
  }
            ...
            <div className="mt-3 flex items-center justify-between border-t pt-3">
              {allowClear ? (
                <button
                  ...
                  disabled={!hasValue}
                  className="inline-flex h-8 items-center justify-center rounded-md px-2 text-xs font-bold text-muted-foreground transition-colors hover:bg-accent hover:text-foreground disabled:pointer-events-none disabled:opacity-40"
                >
                  清除
                </button>
              ) : (
              ...
              <button
                ...
                className="inline-flex h-8 items-center justify-center rounded-md px-2 text-xs font-bold text-primary transition-colors hover:bg-primary/10"
              >
                今天
              </button>
            </div>
