...
type TranslateSettingsPanelProps = {
...
// TranslateSettingsPanel 管理翻译工作流的模型绑定，采用显式保存避免选择时自动提交。
  ...
  return (
    <section className="nexus-surface space-y-4 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-lg font-extrabold text-foreground">Translate 设置</h2>
        </div>
        {dirty && (
      ...
      {workflowsError && (
        <p className="flex items-center gap-1.5 text-sm text-destructive">
          <AlertCircle className="h-4 w-4" /> 加载 Translate 配置失败
        </p>
      )}
      {providers.length === 0 && (
